...
plot_height = function(hc){
  ...
  for (i in 1:treeSize)
    height[i+1]=sqrt(hc_atual$height[treeSize-(i-1)])
  
  dadosx<-data.frame(x=2:20, y=height[2:20], group=1)
  p <- ggplot(dadosx, aes(x,y)) + geom_line(size=1)
  p <- p + xlab('\nNúmero de grupos') + ylab('Distância média dentro dos grupos') 
...
a = centroidesFormatados (hc,mydata,10)
b = centroidesFormatados (hc,mydata,11)

write.csv(unique(rbind(a,b)),paste("C:\\Users\\Andryw\\Dropbox\\Mestrado\\Novos dados\\mestrado\\Dados e Scripts\\Novidades e conhecidas\\Clusters\\centroides_juntos.csv"))
